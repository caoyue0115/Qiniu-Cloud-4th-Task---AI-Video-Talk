...
### 👉 **https://americans-association-understand-deutschland.trycloudflare.com**

打开即是展示页：右侧手机样机里运行**真实 App**（点「接通」体验），左侧实时显示成本、对话记录、四大模式、端云协同架构。

> ⚠️ **说明**：该地址为本地服务 + cloudflared 临时隧道，**仅在作者电脑与隧道运行期间可访问**；隧道重启后地址会变。如链接失效，请按下方「本地运行」自行启动，或联系作者重启隧道。

📱 **手机访问更佳**：用手机浏览器打开同一地址，允许摄像头/麦克风，体验完整通话。
